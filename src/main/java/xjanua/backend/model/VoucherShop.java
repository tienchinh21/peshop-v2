...
@Entity
@Table(name = "voucher_shop")
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class VoucherShop extends BaseEntity {
    @Id
    ...
    @OneToMany(mappedBy = "voucherShop", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    private List<OrderVoucher> orderVouchers;

    @OneToMany(mappedBy = "voucherShop", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    private List<UserVoucherShop> userVoucherShops;
}